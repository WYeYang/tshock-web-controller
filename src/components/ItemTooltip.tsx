...
export const ItemTooltip = ({ item, children }: ItemTooltipProps) => {
  ...
  return (
    ...
    >
      ...
      {isHovered && item.netID > 0 && (
        <div
          className="absolute z-50 pointer-events-none bg-slate-900 border border-slate-700 rounded-lg p-4 shadow-xl min-w-[240px]"
          style={{
            left: Math.max(0, Math.min(position.x, 280)),
            top: position.y,
          }}
        >
